...
//
//		http://ds.serving-sys.com/BurstingRes/CustomScripts/evidon.js?adid=[%tp_adid%]&flightid=[%tp_flightid%]&coid=709&nid=2127&ad_z=999999&position=bottom-right&ecaid=9773663&delay_start=1
//
// Parameters:
//
...
(function() {
    ...
    // Define Ad Choice Object
    var _bao = mdACE[i].initBAO();
    ...
    function AdChoiceEvidon()
    {
        ...
        self.init = function()
        {
            ...
            // 3 ways to determine protocol....
            
            // Using protocol of page that has loaded this JS
            thisPtcl = document.location.protocol == 'https:' ? 'https' : 'http';
            
            // Using same protocol that custom script was using
            // thisPtcl = scriptPath.substr(0,5) == 'https' ? 'https' : 'http';
            // Using query string
            // thisPtcl = gEbQueries.secure == '1' ? 'https' : 'http';
        ...
        // initialize the object that the evidon API requires
        self.initBAO = function() {
            
            var bao = new Object();
            
            if (typeof(self.tagHeight) != "undefined") {
                bao.ad_h = self.tagHeight;
                
            }
            if (typeof(self.tagWidth) != "undefined") {
                bao.ad_w = self.tagWidth;
                
            }
            if (typeof(gEbQueries.coid) != "undefined") {
                bao.coid = gEbQueries.coid;
                
            }
            if (typeof(gEbQueries.nid) != "undefined") {
                bao.nid = gEbQueries.nid;
                
            }
            if (typeof(mmRand) != "undefined") {
                bao.uqid = mmRand;
                
            }
            if (typeof(gEbQueries.position) != "undefined") {
                bao.position = gEbQueries.position;
                
            }
            if (typeof(gEbQueries.ad_z) != "undefined") {
                bao.ad_z = gEbQueries.ad_z;
                
            }
            if (typeof(gEbQueries.ecaid) != "undefined") {
                bao.ecaid = gEbQueries.ecaid;
                
            }
            if (typeof(gEbQueries.delay_start) != "undefined") {
                bao.delay_start = gEbQueries.delay_start;
                
            }
            if (typeof(gEbQueries.adid) != "undefined") {
                bao.container = gEbQueries.adid;
        ...
        self.writeBAO = function() {
            var objParams = {
                ...
                onload: function() {
                    ...
                    // set width and height of banner's parent div
                    
                    //if(self.tagWidth)this.parentNode.style.width = self.tagWidth + "px";
                    
                    //if(self.tagHeight)this.parentNode.style.height = self.tagHeight + "px";
                    imgOnload(this);
                    ...
                    var objParams = {
                        ...
                        {
                            // IE <= 8 require onreadystatechange as they do not support onload for scripts
                             if (self.scriptLoaded ||
                            // in case onreadystatechange already loaded the ad
                            (this.readyState != 'loaded' && this.readyState != 'complete') ||
                            // script is not finished loading
                            (!(displayWin.BAP && displayWin.BAPStart))
                            // BAP object doesn't exist yet 
                            ) return;
                            
                            // when script is loaded, start running it (it doesn't auto-run)
                            displayWin.BAPStart(_bao);
                            self.scriptLoaded = true;
                            
                        },
                        onload: function()
                        {
                            // all browsers other than IE <= 8 can (and should) use onload
                             if (self.scriptLoaded)
                                return;
                            // in case onreadystatechange already loaded the ad
                        ...
                        src: thisPtcl + '://c.betrad.com/surly.js?' + 'ad_w=' + self.tagWidth + '&ad_h=' + self.tagHeight + '&coid=' + gEbQueries.coid + '&nid=' + gEbQueries.nid + '&ecaid=' + gEbQueries.ecaid
        ...
        function mmLoadImageIntoBannerParentDiv(id, imgParams) {
            ...
            //img.setAttribute("src",scriptURL);
